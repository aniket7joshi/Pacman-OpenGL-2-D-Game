...
public:
    Magnet() {}
    Magnet(float x, float y, color_t color, float radius);
    glm::vec3 position;
    float rotation;
...
Magnet::Magnet(float x, float y, color_t color, float radius) {
    ...
    this->object = create3DObject(GL_TRIANGLES,181*3, vertex_buffer_data, color, GL_FILL);  
}
